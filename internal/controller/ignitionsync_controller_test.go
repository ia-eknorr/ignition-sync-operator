...
func newReconciler(gitClient git.Client) *IgnitionSyncReconciler {
	return &IgnitionSyncReconciler{
		Client:    k8sClient,
...
var _ = Describe("IgnitionSync Controller", func() {
	...
	Context("Finalizer handling", func() {
		...
		It("should add finalizer on first reconcile", func() {
			...
			Expect(controllerutil.ContainsFinalizer(cr, synctypes.Finalizer)).To(BeTrue())
		})
		...
		It("should remove finalizer and clean up on deletion", func() {
			...
			cm := &corev1.ConfigMap{
				ObjectMeta: metav1.ObjectMeta{
					Name:      fmt.Sprintf("ignition-sync-metadata-%s", resourceName),
					Namespace: "default",
				},
			...
			Eventually(func() bool {
				err := k8sClient.Get(ctx, nn, &syncv1alpha1.IgnitionSync{})
	...
	Context("Gateway status collection", func() {
		...
		It("should enrich gateways with status from ConfigMap", func() {
			...
			statusData := synctypes.GatewayStatus{
			...
			statusCM := &corev1.ConfigMap{
				ObjectMeta: metav1.ObjectMeta{
					Name:      fmt.Sprintf("ignition-sync-status-%s", resourceName),
					Namespace: "default",
				},
			...
			Expect(gw.SyncStatus).To(Equal(synctypes.SyncStatusSynced))
	...
	Context("Condition aggregation", func() {
		...
		It("should set Ready=True when ref is resolved and all gateways synced", func() {
			...
			statusJSON, _ := json.Marshal(synctypes.GatewayStatus{
				SyncStatus:   synctypes.SyncStatusSynced,
				SyncedCommit: "abc123",
			})
			statusCM := &corev1.ConfigMap{
	...
	Context("Pod to CR mapping", func() {
		It("should map annotated pod to IgnitionSync reconcile request", func() {
			...
			pod := &corev1.Pod{
				ObjectMeta: metav1.ObjectMeta{
					...
					Annotations: map[string]string{
						synctypes.AnnotationCRName: "my-isync",
					},
				},
			}

			requests := r.findIgnitionSyncForPod(context.Background(), pod)
			Expect(requests).To(HaveLen(1))
			Expect(requests[0].NamespacedName.Name).To(Equal("my-isync"))
